...
async function setPower(val) {
    ...
    currentFMStatus.isPowerOn = true;
}
async function getPower() {
    return currentFMStatus.isPowerOn;
}

async function createSerialPort() {
    if (serialPort) return [serialPort, serialPortParser];
...
async function getFMStatus() {
    ...
    data.isCampusOff = result.campos_off;
    data.isPause = result.paus || !result.play;
...
async function sendSerialDataNoQueue(val) {
    await new Promise(function (resolve, reject) {
        if (serialPort.isOpen) {
            debug('Serialport has existed');
            resolve();
            return;
        }
        serialPort.on('open', function () {
            debug('Serialport has opened');
            resolve();
        });
        debug('Opening Serialport');
        serialPort.open();
    });
    return new Promise(function (resolve, reject) {
